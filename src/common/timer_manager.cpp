...
bool timer_manager::add_handler(const std::shared_ptr<timer_handler> &handler,
  ...
  _handlers.insert(std::pair<std::weak_ptr<timer_handler>, timer_set::iterator>(
      handler, timer));
  assert(_expired_set.size() == _handlers.size());
  return true;
}

bool timer_manager::is_repeat_item(
    const std::shared_ptr<timer_handler> &handler) {
...
void timer_manager::remove_handler(
    const std::shared_ptr<timer_handler> &handler) {
  timer_map::iterator it = _handlers.find(handler);
  if (it != _handlers.end()) {
...
bool timer_manager::has_timer_item() { return (_expired_set.size() != 0); }
