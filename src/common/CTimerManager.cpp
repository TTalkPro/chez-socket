...
bool CTimerManager::IsRepeatItem(
    const std::shared_ptr<CTimerTask> &Task) {
...
bool CTimerManager::AddTask(const std::shared_ptr<CTimerTask> &Task,
  ...
  Tasks.insert(std::pair<std::weak_ptr<CTimerTask>, TimerSet::iterator>(
      Task, Timer));
...
void CTimerManager::RemoveFirst() {
  if (ExpiredSet.size() > 0) {
    ...
    // 如果是循环定时，直接再次加回来
    if (Temp.Cycled) {
      const std::shared_ptr<CTimerTask> Task = Temp.Task.lock();
      if (Task != nullptr) {
