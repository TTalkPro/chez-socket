...
void tcp_handler::do_connect(const struct sockaddr *addr, unsigned int addr_len)
{
    ...
    do {
        errno = 0;
        r = connect(fd(), (const sockaddr*)target, addr_len);
...
void tcp_handler::handle_io(int events)
{
    int err = -1;
...
void tcp_handler::make_socket()
{
    ...
    if (fd() == -1)
    {
        if (0 != unix_make_socket(sockfd,_domain,SOCK_STREAM, 0))
            {
